...
export default async function Fixture(match: Match) {
    ...
    return (
        <div aria-label="Fixture" className="flex items-center gap-2 w-full h-[62px] my-4" tabIndex={0}>
            <div className="flex flex-col justify-around text-gray-400">
                <bdi>{match.date.toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}</bdi>
                <bdi>{match.date.toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })}</bdi>
            </div>
